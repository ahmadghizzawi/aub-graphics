...
// TODO: Change obj1 position
// TODO: Change obj2 position
// currently only 1 obj is defined
static Matrix4 g_objectRbt[2] = {Matrix4::makeTranslation(Cvec3(-0.75,0,0)), Matrix4::makeTranslation(Cvec3(0.75,0,0))};
static Cvec3f g_objectColors[2] = {Cvec3f(1, 0, 0), Cvec3f(0, 1, 0)};
// TODO: Change obj2 color
...
// takes MVM and its normal matrix to the shaders (MVM = from local frame to eye frame)
...
static Matrix4 doQtoOwrtA(const Matrix4& Q, const Matrix4& O, const Matrix4& A) {
  // O = A * M * A-1 * O
  return A * Q * inv(A) * O;
}
...
static void objectManipulation(Matrix4 &Q) {
  /* code */
  // Active eye is sky and active object is a cube.
  if ((g_activeObject < 2) && g_activeEye == SKY) {
    Matrix4 A = makeMixedFrame(g_objectRbt[g_activeObject], g_skyRbt);
  ...
  // Active eye and object are cubes.
    ...
    Matrix4 A = makeMixedFrame(g_objectRbt[g_activeObject], g_objectRbt[g_activeEye]);
    g_objectRbt[g_activeObject] = doQtoOwrtA(Q, g_objectRbt[g_activeObject], A);
  }
  // Active eye and object is sky.
    ...
    // World-sky frame
    if (g_worldSky) {
      Matrix4 A = makeMixedFrame(g_worldRbt, g_skyRbt);
...
static void motion(const int x, const int y) {
  ...
  if (g_mouseLClickButton && !g_mouseRClickButton && !g_spaceDown) { // left button down?
    m = Matrix4::makeXRotation(factor2 * factor * -dy) * Matrix4::makeYRotation(factor2 * factor * dx);
  }
  else if (g_mouseRClickButton && !g_mouseLClickButton) { // right button down?
    m = Matrix4::makeTranslation(Cvec3(factor * dx, factor * dy, 0) * 0.01);
  }
    ...
    m = Matrix4::makeTranslation(Cvec3(0, 0, factor * -dy) * 0.01);
  }



  if (g_mouseClickDown) {
    objectManipulation(m);
